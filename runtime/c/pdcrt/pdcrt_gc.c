...
{
    PDCRT_ASSERT(h->grupo == grupo->grupo);
    PDCRT_ASSERT(grupo->grupo != PDCRT_TGRP_NINGUNO);
    if(h->anterior)
...
{
    PDCRT_ASSERT(!h->anterior);
    PDCRT_ASSERT(!h->siguiente);
...
{
    pdcrt_recoleccion params = pdcrt_gc_recoleccion_por_memoria(ctx, sz, true);
...
{
    PDCRT_ASSERT(h->grupo == PDCRT_TGRP_BLANCO_VIEJO
                                          ...
                                          pdcrt_recoleccion params)
{
    ...
    {
        PDCRT_ASSERT(h->grupo == PDCRT_TGRP_GRIS || h->grupo == PDCRT_TGRP_NEGRO);
    }
}
...
static void pdcrt_gc_debe_ser_alcanzable_vis(pdcrt_ctx *ctx, pdcrt_cabecera_gc **h, pdcrt_recoleccion params)
{
    ...
    if(es_mayor)
    {
        PDCRT_ASSERT(g != PDCRT_TGRP_NEGRO || g != PDCRT_TGRP_GRIS);
    }
    else
    {
        PDCRT_ASSERT(g != PDCRT_TGRP_BLANCO_VIEJO || g != PDCRT_TGRP_NEGRO || g != PDCRT_TGRP_GRIS);
    }
}
...
static void pdcrt_gc_no_debe_ser_blanco_vis(pdcrt_ctx *ctx, pdcrt_cabecera_gc **h, pdcrt_recoleccion params)
{
    ...
    if(es_mayor)
    {
        ...
        PDCRT_ASSERT(g != PDCRT_TGRP_BLANCO_VIEJO || g != PDCRT_TGRP_BLANCO_JOVEN || g != PDCRT_TGRP_BLANCO_EN_LA_PILA);
    }
    else
    {
        PDCRT_ASSERT(g != PDCRT_TGRP_BLANCO_JOVEN || g != PDCRT_TGRP_BLANCO_EN_LA_PILA);
    }
}
...
PDCRT_NOINLINE static void pdcrt_gc_procesar_gris(pdcrt_ctx *ctx,
                                                  ...
                                                  pdcrt_recoleccion params)
{
    PDCRT_ASSERT(h->grupo == PDCRT_TGRP_GRIS);
...
#undef PDCRT_X
    ...
    bool es_menor = pdcrt_gc_recoleccion_es_menor(params);
    ...
    if(es_menor)
    {
        for(pdcrt_cabecera_gc *h = ctx->gc.raices_viejas.primero; h != NULL;)
        {
            ...
            PDCRT_ASSERT(h->grupo == PDCRT_TGRP_RAICES_VIEJAS);
...
static size_t pdcrt_gc_recolectar(pdcrt_ctx *ctx, pdcrt_recoleccion params)
{
    ...
    {
        pdcrt_cabecera_gc *s = h->siguiente;
        PDCRT_ASSERT(h->grupo == PDCRT_TGRP_BLANCO_EN_LA_PILA);
    ...
    {
        pdcrt_cabecera_gc *s = h->siguiente;
        PDCRT_ASSERT(h->grupo == PDCRT_TGRP_BLANCO_JOVEN);
    ...
    if(es_mayor)
    {
        ...
        for(pdcrt_cabecera_gc *h = ctx->gc.raices_viejas.primero; h != NULL;)
        {
            pdcrt_cabecera_gc *s = h->siguiente;
            PDCRT_ASSERT(h->grupo == PDCRT_TGRP_RAICES_VIEJAS);
            pdcrt_gc_eliminar_de_grupo(&ctx->gc.raices_viejas, h);
            liberado += pdcrt_gc_liberar_objeto(ctx, h);
            h = s;
        }
...
static void pdcrt_gc_mover_negros_a_blancos(pdcrt_ctx *ctx, pdcrt_recoleccion params)
{
    ...
    {
        PDCRT_ASSERT(h->grupo == PDCRT_TGRP_NEGRO);
        PDCRT_ASSERT(h->tipo != PDCRT_TGC_REUBICADO);
        pdcrt_cabecera_gc *s = h->siguiente;
        if(h->en_la_pila)
...
{
    ctx->cnt += 1;
    pdcrt_recoleccion params = pdcrt_gc_recoleccion_por_pila(ctx, ctx->cnt > 100);
    pdcrt_recolectar_basura_simple(ctx, m, params);
    if(ctx->cnt > 100)
        ctx->cnt = 0;
}
