...
pdcrt_tk pdcrt_recv_archivo(pdcrt_ctx *ctx, int args, pdcrt_k k, PDCRT_F_IMM)
{
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, "Archivo: cerrar no necesita argumentos");
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, "Archivo: estáAbierto no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, "Archivo: leerByte no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, "Archivo: obtenerSiguienteByte no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
        size_t pos = 0;
        ...
        if(ioerr != PDCRT_IO_OK)
            pdcrt_error(ctx, "Archivo#obtenerSiguienteByte: No se pudo posicionar en el archivo");
        unsigned char bytes[1];
        pdcrt_vio_buffer buffer = {
            .ptr = (char *) bytes,
            .cap = 1,
    ...
    {
        if(args != 1)
            pdcrt_error(ctx, "Archivo: escribirByte necesita 1 argumento");
        bool ok = false;
        pdcrt_entero byte = pdcrt_obtener_entero_obj(ctx, pdcrt_obj_desde_xmm(a1), &ok);
        if(!ok)
            pdcrt_error(ctx, "Archivo: escribirByte necesita un entero como argumento");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
        ...
        if(ioerr != PDCRT_IO_OK || escrito == 0)
            pdcrt_error(ctx, "No se pudo escribir el archivo");
        return pdcrt_continuar(ctx, k, pdcrt_xmm_desde_obj(pdcrt_objeto_nulo()));
    }
    else if(pdcrt_comparar_textos(omsj.texto, ctx->textos_globales.escribir_texto))
    {
        if(args != 1)
            pdcrt_error(ctx, "Archivo: escribirTexto necesita 1 argumento");
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, u8"Archivo: posiciónActual no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
        size_t pos = 0;
        pdcrt_io_error ioerr = (*ctx->vio.vtable->op_posicionar)(ctx->vio.ctx, arch->archivo, PDCRT_ARCHIVO_ANCLA_ACTUAL, 0, &pos);
        if(ioerr != PDCRT_IO_OK)
            pdcrt_error(ctx, "No se pudo posicionar el archivo");
    ...
    {
        if(args != 1)
            pdcrt_error(ctx, u8"Archivo: cambiarPosición necesita 1 argumento");
        bool ok = false;
        pdcrt_entero pos = pdcrt_obtener_entero_obj(ctx, pdcrt_obj_desde_xmm(a1), &ok);
        if(!ok)
            pdcrt_error(ctx, u8"cambiarPosición necesita un entero");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, u8"Archivo: cambiarPosición no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, u8"Archivo: error no necesita argumentos");
    ...
    {
        if(args != 0)
            pdcrt_error(ctx, u8"Archivo: __leerTodo no necesita argumentos");
        pdcrt_rsc_archivo *arch = (void *) oyo.valop->datos;
        if(arch->archivo == NULL)
            pdcrt_error(ctx, u8"El archivo no está abierto");
        ...
        while(1)
        {
            ...
            pdcrt_io_error ioerr = (*ctx->vio.vtable->op_leer)(ctx->vio.ctx, arch->archivo, &buffer, 1);
            if(ioerr != PDCRT_IO_OK)
                pdcrt_error(ctx, "Archivo#__leerTodo: no se pudo leer el archivo");
